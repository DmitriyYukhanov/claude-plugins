...
- Opens the PR and stops; once you approve it in-session it merges, preferring a squash and falling back to whatever the repository allows, then tears the branch and worktree down. A merge into anything but the default branch is reported as one, since that is the only case where GitHub closes the issue for you.
...
- Optional `.claude/issue-to-pr/config.md` for board URL, base branch, and test commands (auto-detected when unset); companion skills (`superpowers:writing-plans`, `/codex-collaboration:cross-review`, `humanizer:humanizer`, `ponytail:ponytail`, `ponytail:ponytail-review`, `mattpocock-skills:grilling`) used if installed, with inline fallbacks otherwise.
