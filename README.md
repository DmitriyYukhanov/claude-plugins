...
- Headless: with `--headless` the run's question and its merge approval are comments on the issue, and your reply is picked up by a fresh run that reads the state back from the issue; trivial work self-merges (`--auto-merge`), and a repo can name `human_paths` that never do.
