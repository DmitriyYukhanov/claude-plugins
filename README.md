...
- Headless: with `--headless`, the run's question and its merge approval become comments on the issue. Your reply is picked up by the next `--headless` run you (or a dispatcher) start, which reads the state back from the issue. Trivial work self-merges (`--auto-merge`), and a repo can name `human_paths` that never do.
