...
# (docs/superpowers, .serena). Everything else passes through to the
...
case "$cmd" in
  *"git add"*" -A"* | *"git add"*" --all"* | *"git add"*" ." | *"git add"*" . "* | *"git add"*" :/"*)
    hook_deny "git add -A / --all / . / :/ stages everything, including local-only artifacts (docs/superpowers, .serena). Stage explicit paths instead: git add path1 path2." ;;
esac
