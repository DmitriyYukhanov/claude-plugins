...
| Second-model review (Step 6, `complex`, or any tier once the ratchet has fired) | `/codex-collaboration:cross-review --max-rounds 1 --type code <CHANGED>` — name the files yourself: left alone it resolves a three-dot diff, which is empty until Step 7 commits, and an empty diff is the one case where it stops to ask the user what to review — a fourth contact moment the ask contract does not have. It applies what both models confirm, so re-gate after it and let the ratchet count those findings. | The adversarial subagents, which share the writer's model family: say in the report that no second model read the diff. |
